...
class DiceSet:
    """Handles dice rolling which is treated as a separate object from entities. It also offers functions
       for deciding which die to roll."""
    ...
    def roll_die(self, i: int, blessed: bool) -> int:
        ...
        if len(self.dice_set) > i and not self.dice_set[i].rolled:
    ...
    def roll_die_forced(self, i: int, number: int) -> int:
        ...
        if len(self.dice_set) > i and not self.dice_set[i].rolled:
            return self.dice_set[i].roll(failsafe=False, number=number)
